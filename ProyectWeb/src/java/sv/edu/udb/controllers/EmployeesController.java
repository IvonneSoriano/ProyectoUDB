...
public class EmployeesController extends HttpServlet {
    ...
    private void list(HttpServletRequest request, HttpServletResponse response) {
       HttpSession miSesion = (HttpSession) request.getSession();
        try {
            Integer actualD = (Integer) miSesion.getAttribute("depto");
            Integer actualR = (Integer) miSesion.getAttribute("rol");
            if(actualR == 5){
                request.setAttribute("listEmployees", empDAO.getAll());
            }
            else if( actualR == 1){
                 request.setAttribute("listEmployees", empDAO.getAllByRolAndDepto(2, actualD));
            }
            else{
                 request.setAttribute("listEmployees", empDAO.getAllByRolAndDepto(4, actualD));
            }
    ...
    private void newEmployee(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            List<Deparment> deps = new ArrayList<>();
            switch (actualR) {
                case 5:
                    rols = rolDAO.getAll();
                    deps = depDao.getAll();
