 ...
 * @author
 */
@WebServlet(name = "RequestsController", urlPatterns = {"/request.do"})
        ...
        try (PrintWriter out = response.getWriter()){
            ...
            switch(operacion){
                case "list":
                    ...
                    break;
                case "new":
                    request.getRequestDispatcher("/request/nuevoRequest.jsp").forward(request, response);
                    break;
                case "insert":
...
//    }// </editor-fold>
    private void list(HttpServletRequest request, HttpServletResponse response){
        try {
            ...
            request.getRequestDispatcher("request/listRequest").forward(request, response);
    ...
    private void delete(HttpServletRequest request, HttpServletResponse response){
        try {
            ...
            q.setId(id);
            boolean r = rqDAO.delete(q);
            if (r) {
                request.setAttribute("exito", "Requisito eliminado");
            } else {
                request.setAttribute("fracaso", "Requisito no eliminado");
            }

            request.getRequestDispatcher("/request.do?op=ver").forward(request, response);

        } catch (Exception e) {
            Logger.getLogger("Error in deleteRequest method. Message: " + e.getMessage());
        }
    }
}
